...
class Preprocess:
    def __init__(self, img_h=640, img_w=640, map_labels=None, n_classes=0):
        self.img_h = img_h
        self.img_w = img_w
    ...
    def mantain(self, src, out, n_classes=0, fraction=1, skew=False, normalize=False, max=0, equal=False):
        ...
        if max > 0:
            cl_instances = np.zeros(len(np.where(np.array(self.map_labels) >= 0)[0]), dtype=int)
            cl_limit = 150
            ...
            while np.min(cl_instances) != cl_limit:
                ...
                for f in self.__get_images(imgs, fraction):
                    if f.endswith(".jpg"):
                        ...
                        img = os.path.join(imgs, filename + extension)
                        label = os.path.join(labels, filename +  ".txt")
                        new_img = os.path.join(images_out, filename + "_" + str(appendix) + extension)
                        new_label = os.path.join(labels_out, filename + "_" + str(appendix) + ".txt")
    ...
    def r_crop(self, src, out, n_classes=0, fraction=1, skew=False, normalize=False, max=0, equal=False):
            ...
            if f.endswith(".jpg"):
                ...
                for i in range(0, new_size[0], self.img_h):
                    for j in range(0, new_size[1], self.img_w):
                        ...
                        start_x = i if i + \
                            self.img_h < new_size[0] else new_size[0] - self.img_h
                        start_y = j if j + \
                            self.img_w < new_size[1] else new_size[1] - self.img_w
                        im_crop = (start_x, start_y, start_x +
                                   self.img_w, start_y + self.img_h)
                        im2 = im.crop(im_crop)
                        instances = 0
                        with open(new_label, "w") as f:
                            for line in data:
    ...
    def r_crop_splited(self, src, out, n_classes=0, fraction=1, skew=False, normalize=False, max=0, equal=False):
        if equal:
            cl_instances = np.zeros(self.n_classes, dtype=int)
        ...
        for split in ["train", "val", "test"]:
            ...
            if equal:
                cl_limit = np.max(cl_instances)
                appendix = 0
                while np.min(cl_instances[cl_instances > 0]) != cl_limit:
                    appendix += 1
    ...
    def fill(self, src, out, n_classes=0, fraction=1, skew=False, normalize=False):
        ...
        for f in self.__get_images(images_src, fraction):
            if f.endswith(".jpg"):
                ...
                new_im.paste(
                    im, (int((self.img_w - new_size[0]) / 2),
                ...
                with open(new_label, "w") as f:
                    for line in data:
                        try:
                            ...
                            x = int(float(x) * self.img_w * ratio_w +
                                    (self.img_w - new_size[0]) / 2)
                            y = int(float(y) * self.img_h * ratio_h +
